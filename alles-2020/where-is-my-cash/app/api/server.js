...
app.post('/1.0/login', cors(), function(req, res) {
    ...
    var sql = mysql.format('SELECT api_key FROM general WHERE username=? AND password=?', [username, password]);
...
app.post('/internal/createTestWallet', cors(), function(req, res) {
    ...
    if (ip === "127.0.0.1") {
        ...
        var wallet_id = crypto.randomBytes(20).toString('hex').substring(0,20);
        var query = `INSERT INTO wallets VALUES ('${wallet_id}', NULL, ${balance}, 'TESTING WALLET 1234');`
